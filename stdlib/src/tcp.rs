...
impl TcpModule {
            ...
            Ok(stream) => {
                ...
                match store.lock() {
                    ...
                    Err(e) => Err(anyhow::anyhow!(
                        "failed to acquire connection store lock: {}",
                        e
                    )),
                }
            }
    ...
    fn close(args: &[Val], _env: &Environment, _ctx: &Val) -> Result<Val> {
        if args.len() != 1 {
            return Err(anyhow::anyhow!(
                "close() takes exactly 1 argument: connection_id"
            ));
        }
    ...
    fn set_timeout(args: &[Val], _env: &Environment, _ctx: &Val) -> Result<Val> {
        ...
        match store.lock() {
            Ok(connections) => match connections.get(conn_id) {
                Some(stream_arc) => match stream_arc.lock() {
                    Ok(stream) => match stream.set_read_timeout(Some(timeout)) {
    ...
    fn bind(args: &[Val], _env: &Environment, _ctx: &Val) -> Result<Val> {
        if args.is_empty() || args.len() > 2 {
            return Err(anyhow::anyhow!(
                "bind() takes 1 or 2 arguments: address [, port]"
            ));
        }
        ...
        match StdTcpListener::bind(addr) {
            Ok(listener) => {
                ...
                match store.lock() {
                    ...
                    Err(e) => Err(anyhow::anyhow!(
                        "failed to acquire listener store lock: {}",
                        e
                    )),
                }
            }
    ...
    fn accept(args: &[Val], _env: &Environment, _ctx: &Val) -> Result<Val> {
        if args.len() != 1 {
            return Err(anyhow::anyhow!(
                "accept() takes exactly 1 argument: listener_id"
            ));
        }
    ...
    fn resolve(args: &[Val], _env: &Environment, _ctx: &Val) -> Result<Val> {
        if args.len() != 1 {
            return Err(anyhow::anyhow!(
                "resolve() takes exactly 1 argument: hostname"
            ));
        }
