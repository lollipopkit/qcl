...
mod inlay_hint_tests {
    ...
    use tower_lsp::lsp_types::{Position, Range, InlayHint, InlayHintKind, InlayHintLabel};
    ...
    fn test_param_inlay_hints_commas_inside_strings() {
        ...
        assert_eq!(hints.len(), 2, "unexpected hint count with comma inside string: {:?}", hints);
        let labels: Vec<String> = hints.iter().map(|h| match &h.label {
            tower_lsp::lsp_types::InlayHintLabel::String(s) => s.clone(),
            _ => String::new(),
        }).collect();
    ...
    fn test_param_inlay_hints_multiline_calls() {
        ...
        assert!(labels.iter().any(|l| l.trim() == "a:"), "missing a: in multiline call: {:?}", labels);
        assert!(labels.iter().any(|l| l.trim() == "b:"), "missing b: in multiline call: {:?}", labels);
        assert!(labels.iter().any(|l| l.trim() == "c:"), "missing c: in multiline call: {:?}", labels);
    }
    ...
    fn test_type_inlay_hints_skip_when_annotated() {
        ...
        assert_eq!(labels.iter().filter(|l| l.starts_with(": ")).count(), 1, "expected exactly one type hint, got {:?}", labels);
    }
    ...
    fn test_server_side_like_filtering_for_inlay_hints() {
        ...
        assert!(all_on.iter().any(|h| h.kind == Some(InlayHintKind::PARAMETER)), "expected parameter hints when enabled");
        assert!(all_on.iter().any(|h| h.kind == Some(InlayHintKind::TYPE)), "expected type hints when enabled");

        // 2) Parameters only
        let params_only = filter_hints(combined.clone(), true, false);
        assert!(params_only.iter().all(|h| h.kind == Some(InlayHintKind::PARAMETER)), "only parameter hints should remain");
        ...
        assert!(types_only.iter().all(|h| h.kind == Some(InlayHintKind::TYPE)), "only type hints should remain");
        let t_labels = labels(&types_only);
        assert!(t_labels.iter().any(|l| l.starts_with(": ")) , "expected at least one type label, got {:?}", t_labels);
    ...
    fn test_function_return_type_hints_simple() {
        ...
        assert!(labs.iter().any(|l| l.contains("-> Int")) || labs.iter().any(|l| l.contains("-> Float")) || labs.iter().any(|l| l.contains("-> Any")),
            "expected at least one arrow type hint, got {:?}", labs);
    }
    ...
    fn test_function_return_type_hints_multiple_returns_union() {
        ...
        let labs = labels(&hints);
        assert!(labs.iter().any(|l| l.contains("-> Int | String")) || labs.iter().any(|l| l.contains("-> String | Int")),
            "expected union return type hint (Int | String), got {:?}", labs);
    }
}
