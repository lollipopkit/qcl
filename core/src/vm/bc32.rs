    ...
    pub fn decode(&self) -> Function {
        ...
        while pc < self.code32.len() {
            ...
            if tag == TAG_FOR_RANGE_PREP {
                ...
                code.push(Op::ForRangePrep { idx: a, limit: b, step: c, inclusive, explicit });
                pc += 2;
                continue;
                ...
                code.push(Op::ForRangeGuard { idx: a, limit: b, step: c, inclusive, ofs });
                pc += 2;
                continue;
                ...
                code.push(Op::ForRangeStep { idx: a, step: b, back_ofs });
                pc += 2;
                continue;
...
#[inline]
fn encode_i16(x: i16) -> (u8, u8) { (((x as u16) >> 8) as u8, (x as u8)) }
...
fn encode_op(op: &Op) -> Option<u32> {
    match *op {
        ...
        | Op::CmpLe(d, a, b)
        | Op::CmpGt(d, a, b)
        | Op::CmpGe(d, a, b) if d < 256 && a < 256 && b < 256 => {
            let tag = match op {
        ...
        Op::NullishPick { l, dst, ofs } if l < 256 && dst < 256 && ofs >= -128 && ofs <= 127 =>
        ...
        Op::Call { f, base, argc, retc } if f < 256 && base < 256 && retc == 1 => Some(pack(Tag::Call, f as u8, base as u8, argc)),
        ...
        Op::JmpFalseSet { r, dst, ofs } if r < 256 && dst < 256 && ofs >= -128 && ofs <= 127 =>
            Some(pack(Tag::JmpFalseSet, r as u8, dst as u8, (ofs as i8) as u8)),
        Op::JmpTrueSet { r, dst, ofs } if r < 256 && dst < 256 && ofs >= -128 && ofs <= 127 =>
...
#[inline]
fn sign_extend_24(x: u32) -> i32 { ((x as i32) << 8) >> 8 }
...
pub(crate) fn decode_word(w: u32) -> Op {
    ...
    match tag {
        ...
        x if x == Tag::JmpFalse as u8 => Op::JmpFalse(a, (((b as u16) << 8) | c) as i16),
        x if x == Tag::ToBool as u8 => Op::ToBool(a, b),
        x if x == Tag::Not as u8 => Op::Not(a, b),
        x if x == Tag::Len as u8 => Op::Len { dst: a, src: b },
        x if x == Tag::Index as u8 => Op::Index { dst: a, base: b, idx: c },
        x if x == Tag::JmpIfNil as u8 => Op::JmpIfNil(a, (((b as u16) << 8) | c) as i16),
        x if x == Tag::JmpIfNotNil as u8 => Op::JmpIfNotNil(a, (((b as u16) << 8) | c) as i16),
        x if x == Tag::NullishPick as u8 => Op::NullishPick { l: a, dst: b, ofs: (c as i8) as i16 },
        ...
        x if x == Tag::Call as u8 => Op::Call { f: a, base: b, argc: c as u8, retc: 1 },
        x if x == Tag::LoadCtx as u8 => Op::LoadCtx(a),
        x if x == Tag::JmpFalseSet as u8 => Op::JmpFalseSet { r: a, dst: b, ofs: (c as i8) as i16 },
        x if x == Tag::JmpTrueSet as u8 => Op::JmpTrueSet { r: a, dst: b, ofs: (c as i8) as i16 },
        x if x == Tag::ListSlice as u8 => Op::ListSlice { dst: a, src: b, start: c },
        _ => Op::Jmp(0),
    }
}
...
impl Bc32Function {
    ...
    pub fn try_from_function(f: &Function) -> Option<Self> {
        let n = f.code.len();
        if n == 0 {
            return Some(Self { consts: f.consts.clone(), code32: vec![], n_regs: f.n_regs, protos: f.protos.clone(), param_regs: f.param_regs.clone() });
        }
        ...
        loop {
            ...
            for i in 0..n { pref[i + 1] = pref[i] + words_per_op[i]; }
            for (i, op) in f.code.iter().enumerate() {
                match *op {
                    Op::JmpFalseSet { ofs, .. } | Op::JmpTrueSet { ofs, .. } => {
                        let j = ((i as isize) + ofs as isize) as isize;
                        if j < 0 || j as usize >= n { return None; }
                        let j = j as usize;
                        let wofs = (pref[j] as isize - pref[i] as isize) as i32;
                        let need_two = wofs < -128 || wofs > 127;
                        let old = words_per_op[i];
                        let new = if need_two { 2 } else { 1 };
                        if new != old { words_per_op[i] = new; changed = true; }
                    }
                    _ => {}
                }
            }
        ...
        let mut acc = 0usize;
        for (i, w) in words_per_op.iter().enumerate() { op_to_word[i] = acc; acc += *w; }
        let total_words = acc;
        ...
        for (i, op) in f.code.iter().enumerate() {
            match op {
                ...
                Op::NullishPick { l, dst, ofs } => {
                    if *l >= 256 || *dst >= 256 { return None; }
                    let tgt = ((i as isize) + *ofs as isize) as usize;
                    let wofs = (op_to_word[tgt] as isize - op_to_word[i] as isize) as i32;
                    if wofs >= -128 && wofs <= 127 {
                ...
                Op::JmpFalseSet { r, dst, ofs } => {
                    if *r >= 256 || *dst >= 256 { return None; }
                    let tgt = ((i as isize) + *ofs as isize) as usize;
                    let wofs = (op_to_word[tgt] as isize - op_to_word[i] as isize) as i32;
                    if wofs >= -128 && wofs <= 127 && words_per_op[i] == 1 {
                ...
                Op::ForRangePrep { idx, limit, step, inclusive, explicit } => {
                ...
                Op::ForRangeGuard { idx, limit, step, inclusive, ofs } => {
                    let tgt = ((i as isize) + *ofs as isize) as usize;
                    let wofs = (op_to_word[tgt] as isize - op_to_word[i] as isize) as i16;
        ...
        Some(Self { consts: f.consts.clone(), code32: out, n_regs: f.n_regs, protos: f.protos.clone(), param_regs: f.param_regs.clone() })
    }
}

/// Utility: expose tag and constants for VM bc32 fast-path
pub(crate) fn tag_of(w: u32) -> u8 { ((w >> 24) & 0xFF) as u8 }
...
mod tests {
    ...
    fn test_bc32_roundtrip_simple() {
        let f = Function {
            ...
            code: vec![Op::LoadK(0, 0), Op::Move(1, 0), Op::ToBool(2, 1), Op::Jmp(1), Op::JmpFalse(2, -1)],
            n_regs: 3,
            protos: vec![],
            param_regs: vec![],
            #[cfg(feature = "bc32")]
