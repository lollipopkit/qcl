...
mod tests {
    ...
    fn test_if_let_simple_variable() {
        let ctx = Val::Map(Arc::new([
            ("data".to_string(), Val::Int(42))
        ].into_iter().collect()));

        let result = parse_and_execute_stmt(
            "if let x = @data { return x; }",
            &ctx
        ).unwrap();
    ...
    fn test_if_let_list_destructuring() {
        let ctx = Val::Map(Arc::new([
            ("list".to_string(), Val::List(Arc::new(vec![
    ...
    fn test_if_let_list_with_rest() {
        ...
        let result = parse_and_execute_stmt(
            "if let [first, ..rest] = @list { return first; }",
            &ctx
        ).unwrap();
    ...
    fn test_if_let_map_destructuring() {
        ...
        let result = parse_and_execute_stmt(
            r#"if let {"name": name} = @user { return name; }"#,
            &ctx
        ).unwrap();
    ...
    fn test_if_let_wildcard() {
        ...
        let result = parse_and_execute_stmt(
            "if let _ = @data { return 1; } else { return 0; }",
            &ctx
        ).unwrap();
    ...
    fn test_if_let_nested_pattern() {
        let ctx = Val::Map(Arc::new([
            ("data".to_string(), Val::Map(Arc::new([
                ("items".to_string(), Val::List(Arc::new(vec![
                    Val::Str("first".into()), Val::Str("second".into())
                ])))
            ].into_iter().collect())))
        ].into_iter().collect()));
